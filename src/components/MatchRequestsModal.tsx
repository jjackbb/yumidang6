...
export const MatchRequestsModal: React.FC<MatchRequestsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-xs p-0 sm:p-4 animate-in fade-in duration-200">
      ...
      >
        {/* Header */}
        <div className="px-5 py-4 flex items-center justify-between shadow-xs shrink-0 z-10">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                대기 중인 신청
                <span className="bg-[#6c2cf5] text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
            ...
            {pendingRequests.length === 0 ? (
              ...
              <div className="space-y-3">
                {pendingRequests.map((req) => (
                  <div
                    key={req.id}
                    className="p-4 rounded-2xl bg-white shadow-xs hover:shadow-md transition-all space-y-3"
                  >
                    {/* Post Title Badge */}
                    ...
                    {/* Intro Message */}
                    <div className="p-3 bg-gray-50 rounded-xl text-xs text-gray-700 leading-relaxed">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        ...
                        className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5"
                      >
            ...
            <div>
              ...
              <div className="space-y-2">
                {pastRequests.map((req) => (
                  <div
                    key={req.id}
                    className="p-3 rounded-xl bg-gray-50 flex items-center justify-between text-xs opacity-75"
                  >
                    <div className="flex items-center gap-2">
                      <img
                        src={req.requesterAvatar}
                    ...
                    <div>
                      {req.status === 'accepted' ? (
                        ...
                        <span className="text-[11px] font-semibold text-gray-400 bg-gray-200 px-2.5 py-0.5 rounded-full">
                          거절됨
                        </span>
                      )}
                    </div>

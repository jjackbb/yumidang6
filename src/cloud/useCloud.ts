...
export function useCloud(enabled: boolean, userId: string | null, authLoading: boolean, apply: (data: PrototypeData) => void) {
 const [loadedFor,setLoadedFor]=useState<string|undefined>();const [error,setError]=useState('');const [busy,setBusy]=useState(false);
 ...
 const refresh=useCallback(async()=>{
  ...
  try{
   ...
   applyRef.current(data);setLoadedFor(identity);setError('');
  }catch(e){if(current.current===identity)setError(e instanceof Error?e.message:'데이터를 불러오지 못했어요.');}
  finally{pending.current=false;}
 },[enabled,userId,identity,authLoading]);
 useEffect(()=>{
  if(!enabled)return;
  generation.current++;setLoadedFor(undefined);applyRef.current(emptyCloudData());
  // Requests started for a previous identity must finish before the next load.
  const retry=setInterval(()=>{if(!pending.current)void refresh();},8000);
  void refresh();const focus=()=>void refresh();window.addEventListener('focus',focus);
  return()=>{generation.current++;clearInterval(retry);window.removeEventListener('focus',focus);};
 },[refresh,enabled]);
 const run=async(action:string,data:Record<string,unknown>={},after?:(result:{id?:string;roomId?:string})=>void)=>{
  if(mutation.current)return false;mutation.current=true;setBusy(true);setError('');const actor=current.current;
  try{
 ...
 return {ready:!enabled||(!authLoading&&loadedFor===identity),error,busy,refresh,run};
}

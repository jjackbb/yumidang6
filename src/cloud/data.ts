...
export function mapCloudData(t: Record<string, any[]>, userId: string | null): PrototypeData {
  ...
  d.rooms=list('chat_rooms').map(c=>{
    ...
    const proposals=list('schedule_proposals').filter(s=>s.appointment_id===a?.id).map(s=>({id:s.id,senderId:s.proposer_id,text:'일정·장소 변경 제안',createdAt:s.created_at,proposal:{startsAt:s.starts_at,endsAt:s.ends_at,newDateTime:formatMeetupRange(s.starts_at,s.ends_at),newLocation:s.proposed_location,status:s.status==='cancelled'?'rejected':s.status}}));

...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (index < 0 || code !== '123456') return Response.json({ error: '안내된 테스트 번호와 인증번호 123456을 입력해 주세요.' }, { status: 401, headers });
    const url = Deno.env.get('SUPABASE_URL')!;
    const secret = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const admin = createClient(url, secret, { auth: { persistSession: false, autoRefreshToken: false } });
    const email = `prototype-${index + 1}@yumidang.invalid`;
    const key = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
    const bytes = new Uint8Array(await crypto.subtle.sign('HMAC', key, new TextEncoder().encode('yumidang-test-v1:' + email)));
    ...
    const created = await admin.auth.admin.createUser({ email, password, email_confirm: true, app_metadata: { prototype_account: true }, user_metadata: { prototype_label: `테스트 ${index + 1}` } });
    ...
    if (error || !data.session || !data.user?.app_metadata?.prototype_account) throw error || new Error('테스트 계정을 확인할 수 없어요.');
    const id = data.user.id;
    const profile = await admin.from('profiles').upsert({ id, display_name: `테스트 ${index + 1}`, bio: 'DB 연결을 확인하는 공용 테스트 계정입니다.', neighborhood: '성동구 성수동', age_group: '20대', hobbies: ['산책', '전시'], traits: ['차분한'], avatar_url: '/logo.jpg' }, { onConflict: 'id', ignoreDuplicates: true });
    if (profile.error) throw profile.error;
    const privateProfile = await admin.from('private_profiles').upsert({ user_id: id, gender: 'undisclosed', terms_version: 'prototype', terms_accepted_at: new Date().toISOString() }, { onConflict: 'user_id', ignoreDuplicates: true });
    ...
    return Response.json({ error: '테스트 로그인에 실패했어요. 잠시 후 다시 시도해 주세요.' }, { status: 400, headers });
  }
});

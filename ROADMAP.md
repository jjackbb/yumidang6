...
2. **신뢰 지표 '당도'**: 매너온도 대신 **당도 99.2 🍯** 체계를 기반으로 상호 평가를 정산합니다.
...
- [ ] **3-1. 참여 요청 발송 및 소개 메시지 작성** `[P0]`
  - 게스트가 공고에 1:1 참여 신청 시, 간단한 소개와 인사 메시지 첨부
- [ ] **3-2. 호스트의 받은 요청 목록 확인 및 수락/거절 (`MyPageView.tsx`)** `[P0]`
  - 신청자의 프로필, **당도**, 후기를 비교하여 수락 또는 정중한 거절 선택
- [ ] **3-3. 1:1 단일 확정 트랜잭션 (Single Lock)** `[P0]`
  - 1명의 요청을 수락하는 즉시 **`2/2명 매칭완료`**로 잠금
  - 해당 공고의 다른 대기 중인 참여 요청은 자동으로 '마감 안내' 처리
- [ ] **3-4. 1:1 채팅방 내 조건 변경 제안 카드 (`ChatView.tsx`)** `[P1]`
  - 약속 일시 또는 장소 변경 시 채팅창 내에 `[일정 변경 제안]` 카드 렌더링
  - 상대방이 [동의] 시 일정 자동 업데이트, [거절] 시 기존 일정 유지
- [ ] **3-5. 일정 중복 충돌 경고** `[P1]`
  - 이미 다른 1:1 동행이 확정된 시간대와 겹칠 경우 신청/수락 전 경고 모달 표시
...
- [ ] **4-1. 만남 전 안심 안전 수칙 팝업 (`SafetyRulesModal.tsx`)** `[P0]`
  - 공공장소 만남, 음주 강요 금지, 비상 연락망 등록 안내
- [ ] **4-2. 가상 안심 음성 통화 시뮬레이션 (`VoiceCallModal.tsx`)** `[P0]`
  - 개인 전화번호 노출 없는 1:1 앱 내 통화
  - 통화 연결음, 음소거, 통화 종료 인터페이스 구현
- [ ] **4-3. 긴급 신고 및 노쇼(No-Show) 접수 센터 (`ReportModal.tsx`)** `[P0]`
  - 약속 장소에 20분 이상 미출현 시 [노쇼 신고] 버튼 활성화
  - 비매너/위험 행동 즉시 신고 접수 및 운영진 제재 플래그
- [ ] **4-4. D-Day 및 1시간 전 약속 리마인더 알림** `[P1]`
  - 약속 전날 및 약속 1시간 전 자동 리마인더 카드 생성

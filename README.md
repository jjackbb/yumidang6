...
인증 업데이트 (2026-09-15): [테스트 로그인 사용법과 검증 범위](docs/auth/USER_REVIEW.md). 현재 요청에 따라 로컬 환경은 **프로토타입 인증**(`123456`)을 사용한다. Supabase 실제 인증과는 구분되며 `/chat`, `/me`에 로그인 확인을 적용했다.

DB 업데이트 (2026-09-15): 선택한 Supabase `yumidang` 프로젝트에 기본 테이블 19개와 RLS를 적용했다. **현재 Vercel 배포 앱과 새 DB의 업무 데이터 연동은 아직 완료하지 않았다.** [스키마·권한·검증·배포 연결 상태](docs/database/HANDOFF.md)를 참고한다.

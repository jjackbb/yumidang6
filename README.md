...
현재 작업 기준 (2026-09-15): 사용자 요청으로 **Claude Opus 5 단계별 묶음 구현을 시작**한다. [밤샘 구현 시작 안내](docs/overnight/START_HERE.md) → [실행 상태](docs/overnight/STATUS.md) → [아침 확인 보고서](docs/overnight/MORNING_REPORT.md)를 확인한다. 확정된 화면·정책을 먼저 연결하고 디자인 통일은 후속으로 진행한다. 아래 소개는 기존 프로젝트 설명이며 최신 정책의 확정 근거가 아니다.

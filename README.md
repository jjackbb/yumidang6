...
인증 업데이트 (2026-09-15): 로컬 환경은 **DB 테스트 인증**(`VITE_AUTH_MODE=supabase-test`)을 사용한다. 공용 테스트 번호 `01000000001`~`01000000003`과 코드 `123456`으로 실제 Supabase 세션을 발급한다. 실제 SMS 본인인증은 아니다. [사용 안내](docs/database/USER_REVIEW.md).

DB 업데이트 (2026-09-15): `yumidang` 프로젝트의 테이블 19개와 RLS에 업무 API·로컬 앱을 연결했다. 실제 계정 간 공고 작성·신청·수락 검증을 통과했다. **마지막 채팅 버튼 수정의 브라우저 재검증과 새 Vercel 배포는 대기 중이다.** [최신 구현·검증·배포 상태](docs/database/INTEGRATION.md)를 참고한다.

현재 작업 기준 (2026-09-15): Claude 사용량 한도로 **GPT가 남은 4~6단계를 이어받도록 전환**했다. [GPT 인수인계](docs/overnight/GPT_HANDOFF.md) → [실행 상태](docs/overnight/STATUS.md)를 확인한다. Supabase DB 작업은 다른 대화 세션에서 진행 중이다. 확정된 화면·정책을 먼저 연결하고 디자인 통일은 후속으로 진행한다. 아래 소개는 기존 프로젝트 설명이며 최신 정책의 확정 근거가 아니다.
